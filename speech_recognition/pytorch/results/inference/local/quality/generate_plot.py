...
if __name__ == "__main__":
    ...
    for run_num, audioname in enumerate(run_num_to_audioname):
        color = 'r' 
        stat = {'idx': audio_stats[audioname][0], 'dur': audio_stats[audioname][1]}
        stat.update(res[run_num])
...
#         plt_show_no_blk(t=0.0001)
    ...
    plt.title('LOCAL GPU: Scatter plot of inference trials and variances of Librispeech Test Clean inputs')
...
#     plt.savefig("sample_runtime.jpg")
    ...
    plt.title('LOCAL GPU: Normalized mean performance plot Librispeech Test Clean inputs')
    plt.xlabel('Idx of input')
    ...
    hist, bin_edges = np.histogram(res,bins=70)
    ...
    plt.title('LOCAL GPU: % of batch size 1 inputs from Librispeech Test Clean satisfying a latency bound')
