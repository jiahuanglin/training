...
if __name__ == "__main__":
    ...
    for i, bound in enumerate(slowdowns_bound):
        ax.errorbar(bound,slowdowns_bounded_throughputs[i],xerr=slowdowns_bounded_throughputs_xerr[i],yerr=slowdowns_bounded_throughputs_yerr[i])
    z = np.polyfit(slowdowns_bound[:], slowdowns_bounded_throughputs[:], 6)
    plt.title('Azure E16_v3 CPU: Librispeech Test Clean Workload Performance (normalized by batch 1 latency)')
